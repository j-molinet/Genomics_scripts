...
for G in "${GENOMAS[@]}"; do
    ...
    # Run BLAST and keep the best hit
    ...
    if [ $FLANK_START -lt 1 ]; then
        FLANK_START=1
    fi
    FLANK_END=$(( END + FLANK ))

    REGION="${SSEQID}:${START}-${END}"
    REGION_FLANK="${SSEQID}:${FLANK_START}-${FLANK_END}"

    # Extract gene sequence
    samtools faidx $GENOMA $REGION > ${G}_hit_tmp.fasta
    # Extract sequence with flanks
    samtools faidx $GENOMA $REGION_FLANK > ${G}_flank_tmp.fasta

    # Reverse-complement if needed
    if [ $REVCOMP -eq 1 ]; then
        seqkit seq -r -p ${G}_hit_tmp.fasta > ${G}_hit.fasta
        seqkit seq -r -p ${G}_flank_tmp.fasta > ${G}_flank.fasta
        rm ${G}_hit_tmp.fasta ${G}_flank_tmp.fasta
    else
        mv ${G}_hit_tmp.fasta ${G}_hit.fasta
        mv ${G}_flank_tmp.fasta ${G}_flank.fasta
    fi

    echo "Best hit sequence saved in: ${G}_hit.fasta"
    echo "Best hit with ±${FLANK} bp flanks saved in: ${G}_flank.fasta"
done
